...
var bundlePackageMigration = &Migration{
	...
	Up: func(ctx context.Context, tx *sql.Tx) error {
		...
		CREATE TABLE operatorbundle_new (
			...
			replaces TEXT,
			skips TEXT,
			FOREIGN KEY(package_name) REFERENCES package(name) DEFERRABLE INITIALLY DEFERRED
		);
		CREATE UNIQUE INDEX pk ON operatorbundle(name, version, bundlepath);
		CREATE UNIQUE INDEX pk ON operatorbundle(name, package_name);
		`
		...
		CREATE TABLE channel_entry_new (
			...
			FOREIGN KEY(channel_name) REFERENCES channel(name) DEFERRABLE INITIALLY DEFERRED,
		...
		foreignKeyOn := `PRAGMA foreign_keys = ON`
